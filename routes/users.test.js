...
describe("POST /users", function () {
  test("authorized for admin: create non-admin", async function () {
  ...
  test("authorized for admin: create admin", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
...
describe("GET /users/:username", function () {
  ...
  test("unauth to get user info of different user", async () => {
    const resp = await request(app)
        .get(`/users/u2`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("PATCH /users/:username", () => {
  ...
  test("unauth to update user info of different user", async () => {
    const resp = await request(app)
        .patch(`/users/u2`)
        .send({
          firstName: "New",
